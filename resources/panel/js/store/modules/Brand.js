...
const actions = {
	getItems({commit},api_url){
		api_url = api_url || '/api/panel/'+itemName;
		axios.get(api_url)
			.then(response=>{
	...
	deleteItem({commit},id){
		axios.delete('/api/panel/'+itemName+'/'+id)
			.then(response=>{
				response = response.data;
				if (response.success){
	...
	addItem({commit},data){
		...
		axios.post('/api/panel/'+itemName,formData,{
			headers: {
	...
	updateItem({commit,state},data){
		...
		axios.post('/api/panel/'+itemName+'/'+data.id,formData,{
			headers: {
              'Content-Type': "multipart/form-data; charset=utf-8; boundary=" + Math.random().toString().substr(2)
            }
		})
	...
	updateStatus({commit,state},data){
			...
			axios.put('/api/panel/'+itemName+'/status/'+data.id+'/'+data.status)
				.then(response=>{
					response = response.data;
					if (response.success) {
